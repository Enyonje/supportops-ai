...
const StatCard = ({ title, value, icon: Icon }) => (
  <div className="group rounded-2xl bg-white/5 border border-white/10 p-6 hover:bg-white/10 transition-all cursor-pointer">
    ...
    <h2 className="text-5xl font-extrabold mt-4 text-white">{value}</h2>
  </div>
);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md no-print">
      <div className="bg-slate-900 border border-white/10 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
          <div>
            <h3 className="text-xl font-bold text-white">{ticket.ticket || ticket.id}</h3>
        ...
        <div className="p-6 h-80 overflow-y-auto space-y-4 bg-slate-950/50">
          <div className="flex gap-3">
            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center shrink-0"><User size={14} className="text-white"/></div>
          ...
          <div className="flex gap-3 flex-row-reverse">
            <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center shrink-0"><Bot size={14} className="text-white"/></div>
        ...
        <div className="p-6 border-t border-white/10 bg-white/5">
          ...
          <button onClick={() => {toast.success("Manual override successful!"); onClose();}} className="w-full mt-4 bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-xl font-bold transition-all">Resolve Manually</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ backgroundColor: '#020617' }} className="min-h-screen text-white flex font-sans">
          ...
          {activeTab === "Dashboard" && (
            <>
              <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                ...
                {isDemoMode && (
                  <div className="group rounded-2xl bg-blue-500/10 border border-blue-500/20 p-6 animate-pulse">
          ...
          {activeTab === "Analytics" && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-right-4 duration-700">
              <div className="bg-white/5 border border-white/10 p-8 rounded-3xl">
                ...
                <div className="grid grid-cols-3 gap-4 mb-10">
                  <div className="p-4 bg-white/5 rounded-2xl border border-white/5 text-center">
                    ...
                    <p className="text-xl font-black text-white">{MOCK_ANALYTICS.kpi_metrics.avg_response_time}</p>
                  </div>
                  <div className="p-4 bg-white/5 rounded-2xl border border-white/5 text-center">
                    <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">CSAT</p>
                    <p className="text-xl font-black text-white">{MOCK_ANALYTICS.kpi_metrics.customer_csat}</p>
                  </div>
                ...
                <div className="flex items-end justify-between h-32 gap-2">
                  {MOCK_ANALYTICS.traffic_volume.map(day => (
                    <div key={day.day} className="flex-1 flex flex-col items-center gap-2">
                      <div className="w-full bg-blue-600/40 rounded-t-md hover:bg-blue-600 transition-all" style={{ height: `${(day.tickets/250)*100}%` }}></div>
              ...
              <div className="bg-white/5 border border-white/10 p-8 rounded-3xl">
                ...
                <button onClick={handleExportPDF} className="w-full mt-10 py-4 rounded-2xl border border-blue-500/30 bg-blue-500/5 text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2 text-white hover:bg-blue-500/10 transition-all no-print">
      ...
      <footer className="fixed bottom-6 left-64 right-10 pointer-events-none md:flex hidden no-print">
        <form className="relative w-full max-w-3xl mx-auto pointer-events-auto" onSubmit={(e) => { e.preventDefault(); toast.success("AI is processing query..."); setClientMessage(""); }}>
          <input className="w-full bg-slate-900 border border-white/10 p-5 rounded-2xl outline-none focus:border-blue-500 shadow-2xl text-slate-200" placeholder="Ask AI to analyze logs..." value={clientMessage} onChange={(e) => setClientMessage(e.target.value)} />
          <button className="absolute right-3 top-3 bottom-3 px-6 bg-blue-600 rounded-xl hover:bg-blue-500 transition-colors text-white"><Send size={18} /></button>
        </form>
      </footer>
...
// --- FINAL APP EXPORT (Only one 'App' declaration here) ---
